...
class Logger {
  ...
  // private methods are prefixed with __ instead of # since standard can't deal with that
  ...
  __log (message) {
    console.log(this.__timestampify(message))
  }

  __err (message) {
    console.error(this.__timestampify(message))
  }

  debug (object) {
    if (this.v0 >= 2) {
      this.__log(JSON.stringify(object))
    }
  }
